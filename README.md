...
- The app uses Laravel's default `web` middleware stack (including session middleware) from framework defaults; only the custom `api.token` alias is registered in `bootstrap/app.php`.
